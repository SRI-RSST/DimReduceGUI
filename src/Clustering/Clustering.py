...
class setcluster(object):
    def __init__(self, clusternum, datafilt, plot_data, labels, group):
        ...
        btn_ok.clicked.connect(
            lambda: self.confirmed_cluster(clusterset, datafilt, clusterset.value(), plot_data, labels, labelc1,
                                           labelc2, labelc3, group) if group == "Treatment" and len(
                np.unique(labels)) > 1 else self.confirmed_cluster(clusterset, datafilt, clusterset.value(), plot_data,
                                                                   labels, None, None, None, group))
    ...
    def confirmed_cluster(self, num, datafilt, numclusters, plot_data, labels, labelc1, labelc2, labelc3, group):
        if num.value() > 0:
            ...
            if group == "Treatment" and len(np.unique(labels)) > 1:
                clusters, counts, idx = Clustering().computeClustering(datafilt, numclusters,
                ...
                labelc2.setText(
                    'Normalized mutual information: ' + str(met.normalized_mutual_info_score(treatlabels, idx)))
                labelc3.setText(
                    'Adjusted mutual information: ' + str(met.normalized_mutual_info_score(treatlabels, idx)))
        else:
        ...
        if numclusters != None:
            ...
            if name[0] != '':
                ...
                clusters, count, idx = Clustering().computeClustering(datafilt, numclusters,
                                                                      np.array(list(zip(plot_data[0], plot_data[1]))))
                # get info from feature
                if 'csv' in featurefile:
                    data = pd.read_csv(featurefile, na_values='NaN')
                else:
                    data = pd.read_csv(featurefile, sep='\t', na_values='NaN')
                data.dropna(axis=0, inplace=True)
                try:
                    data['Cluster Assignment'] = idx
                    # export cluster info + featurefile info
                    if 'csv' in featurefile:
                        data.to_csv("".join(name), mode='w', index=False)
                    else:
                        data.to_csv("".join(name), sep='\t', mode='w', index=False)
                except Exception as ex:
                    errorWindow('Cluster Export',
                                "Check Validity of Feature File. \n Python Exception: {}".format(ex))
        else:
...
class Clustering:
    ...
    # computeClustering.m
    def computeClustering(self, data, numberClusters, projection_data, type='AP', sparse=False, maxits=500, convits=15,
                          dampfact=0.5,
        ...
        while not dn:
            ...
            if plot or details:
                if K == 0:
                    ...
                    tmpidx = I[c]
                    s_flat = S.flatten()
                    tmpnetsim = np.sum(s_flat[(np.dot(np.array(tmpidx - 1), N) + np.arange(0, N).T)],
                                       axis=0)  # might not need axis here
